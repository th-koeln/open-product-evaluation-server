...
describe('User', () => {
  ...
  beforeEach(async () => {
    ...
    const { errors: secondInactiveErrors } = await authRequest(secondInactiveQuery, adminToken)
  ...
  describe('should', async () => {
    ...
    it('create a new version when creating question and votes are present',async () => {
      ...
      const getQuery = resultsQuery(getSeedID(surveys[1]))
    ...
    it('create a new version when creating item and votes are present',async () => {
      const query = createItemMutation(getSeedID(questions[6]), 'New item')
      const { errors } = await authRequest(query, jwt)
