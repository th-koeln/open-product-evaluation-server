...
describe('User', () => {
  ...
  describe('should', async () => {
    it('get the amount of all owned / created surveys', async () => {
      const query = surveyAmountQuery()
      const res = await authRequest(query, jwt)
      const { data, errors } = res

      expect(errors).toBeUndefined()
      expect(data.surveyAmount).toBe(1)
    })
    it('get all owned / created surveys when querying for surveys', async () => {
    ...
    it('get owned / created survey and see creator when querying for a specific survey', async () => {
    ...
    it('update description of owned inactive survey to be empty', async () => {
      const query = updateSurveyMutation(
        getSeedID(surveys[1]),
  ...
  describe('should not', async () => {
    ...
    it('get not owned / created survey when querying for a specific survey', async () => {
    ...
    it('update isActive of not owned survey', async () => {
      const query = updateSurveyMutation(
        getSeedID(surveys[0]),
        { isActive: false }
      )
    ...
    it('update title of not owned survey', async () => {
      const query = updateSurveyMutation(
        getSeedID(surveys[0]),
        { title: 'New title', isActive: false }
      )
    ...
    it('update description of not owned survey', async () => {
      const query = updateSurveyMutation(
        getSeedID(surveys[0]),
        { description: 'New description', isActive: false }
      )
    ...
    it('update questionOrder of not owned survey', async () => {
      const query = updateSurveyMutation(
        getSeedID(surveys[0]),
        {
          questionOrder: [
    ...
    it('delete not owned / created survey', async () => {
